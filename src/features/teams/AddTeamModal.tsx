...
import { createTeamSchema, type CreateTeamFormValues } from '@/schemas/team.schema';
...
/** Admin-only "create a team" form — POST /teams, then the parent list reloads. */
    ...
    <Modal isOpen={isOpen} onClose={handleClose} title="Add team">
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
        <div>
          ...
          />
          {errors.name ? (
            <p className="mt-1.5 text-sm text-danger-600">{errors.name.message}</p>
          ) : null}
        </div>
